...
namespace mod
{
    void ChestRandomizer::generate()
    {
        ...
        for (u16 i = 0; i < sizeof(item::checkPriorityOrder) / sizeof(u16); i++)
        {
            ...
            if (!destCheck->source)
            {
                ...
                {
                    sourceCheck = findSource(destCheck->destLayer, 0x1,
                                             destCheck);  // to prevent woodensword from being overwritten before losing it
                }
                else if (destCheck->itemID == items::Item::Ordon_Shield || destCheck->itemID == items::Item::Wooden_Shield ||
                         destCheck->itemID == items::Item::Hylian_Shield)
                {
                    sourceCheck =
                        findSource(destCheck->destLayer, 0x2,
                ...
                {
                    sourceCheck = findSource(destCheck->destLayer, 0x7,
                ...
                if (destCheck->destLayer != 0xFF)
                {
                    ...
                    if (destCheck->itemID == items::Item::Ordon_Sword)
                    {
                        sourceCheck = findSource(destCheck->destLayer, 0x1,
                             ...
                             destCheck->itemID == items::Item::Wooden_Shield || destCheck->itemID == items::Item::Hylian_Shield)
                    {
                        sourceCheck =
                            findSource(destCheck->destLayer, 0x2,
                                       destCheck);  // to prevent softlocking the game when you try to get ordon shield check
                    ...
                    else if (isProgressiveEnabled == 0)
                    {
                        ...
                        {
                            sourceCheck = findSource(destCheck->destLayer, 0x6,
        ...
        /*for(u16 i = 0; i < totalChecks; i++)
        {
                ...
                if(!destCheck->source)
                {
                        ...
                        if(item::getFlags(destCheck->itemID, 0) != 0)
                        {
                                ...
                                if (destCheck->itemID == items::Item::Ordon_Shield || destCheck->itemID ==
        items::Item::Wooden_Shield || destCheck->itemID == items::Item::Hylian_Shield)
                                {
                                        sourceCheck = findSource(0xFF, 0x2, destCheck);//to prevent softlocking the game when
        you try to get ordon shield check
                                }
                                else
                                {
            ...
            if (!destCheck->source)
            {
                    ...
                    destCheck->itemID == items::Item::Hylian_Shield)
                {
                    sourceCheck = findSource(
                        0xFF, 0x2, destCheck);  // to prevent softlocking the game when you try to get ordon shield check
                }
                else
                {
                    sourceCheck = findSource(0xFF, 0, destCheck);
    ...
    item::ItemCheck* ChestRandomizer::findSource(u8 maxLayer, u8 minLayer, item::ItemCheck* destCheck)
    {
        ...
        } while (!checkCondition(sourceCheck, destCheck) || sourceCheck->destination || sourceCheck->sourceLayer > maxLayer ||
                 sourceCheck->sourceLayer < minLayer ||
                ...
                (tp::d_a_alink::checkStageName("F_SP128") && 0 == strcmp(sourceCheck->stage, "R_SP128")))
            {
                if (isGrottoCheckOk(i))
                {
                         ...
                         (item == items::Item::Simple_Soup || item == items::Item::Good_Soup)))
                    {
                        ...
                        if (isOk)
                        {
                            snprintf(lastSourceInfo, 50, "%s->%d->%x", sourceCheck->stage, sourceCheck->room,
                                     sourceCheck->itemID);
                            if (sourceCheck->destination)
                            {
                                snprintf(lastDestInfo, 50, "%s->%d->%x", sourceCheck->destination->stage,
                                         sourceCheck->destination->room, sourceCheck->destination->itemID);
                                    ...
                                    sourceCheck->itemID == items::Item::Heart_Container)
                                {
                                    sourceCheck->destination = &item::checks[263];  // green rupee
                                         ...
                                         sourceCheck->type !=
                                             item::ItemType::Equip)  // some checks are called twice i don't wanna list them
                                                                     // all, but dungeon items, gear, and equipable items only
                                                                     // have one check each intheir stage
                                {
                                    // Unset this check
                                ...
                                if (isProgressiveEnabled == 1)
                                {
                                    ...
                                    else if (item == items::Item::Giant_Wallet && !tools::checkItemFlag(ItemFlags::Big_Wallet))
                                    {
    ...
    bool ChestRandomizer::isItemBombs(u8 itemID)
    {
        if (itemID == items::Item::Bombs_5 || itemID == items::Item::Bombs_10 || itemID == items::Item::Bombs_20 ||
            ...
            itemID == items::Item::Water_Bombs_15 || itemID == items::Item::Bomblings_5 || itemID == items::Item::Bomblings_10)
        {
            return true;
        }
        else
    ...
    bool ChestRandomizer::isItemBottleFill(u8 itemID)
    {
        if (itemID == items::Item::Red_Potion_Shop || itemID == items::Item::Green_Potion ||
            ...
            itemID == items::Item::Hot_Spring_Water_Shop || itemID == items::Item::Fairy_Tears || itemID == items::Item::Worm ||
            itemID == items::Item::Bee_Larva_Scooped || itemID == items::Item::Rare_Chu_Jelly ||
            ...
            itemID == items::Item::Purple_Chu_Jelly || itemID == items::Item::Simple_Soup || itemID == items::Item::Good_Soup ||
            itemID == items::Item::Superb_Soup || itemID == items::Item::Bee_Larva_Shop ||
