...
namespace mod
{
    ...
    void HUDConsole::draw()
    {
        ...
        // Print heading
        sprintf(console->consoleLine[i].line, "[%d/%d][%-12s]  %s (C) %s", selectedPage + 1, numPages, pages[selectedPage],
                VERSION, AUTHOR);
        ...
        for (i = 1; i <= numOptions[selectedPage]; i++)
        {
            ...
            if (o.Limit == 1)
            {
                sprintf(console->consoleLine[i].line, "%c%-25s %s", (optionindex == selectedOption ? '>' : ' '), o.Title,
                        (*o.Target ? "Yes" : "No"));
            }
            else
            {
                sprintf(console->consoleLine[i].line, "%c%-25s %02x", (optionindex == selectedOption ? '>' : ' '), o.Title,
                        *o.Target);
            }
        }
