...
hudConsole->addWatch(page, "throw:", &throwResult, 'x', WatchInterpretation::_u16);*/
        ...
        eventListener->addLoadEvent(stage::allStages[Stage_Ordon_Village], 0x1, 0x15, 0xFF, 0xFF, game_patch::unlockBossDoors,
                                    event::LoadEventAccuracy::Stage_Room);

        // early CiTS
        eventListener->addLoadEvent(stage::allStages[Stage_Lake_Hylia], 0xFF, 0xFF, 0xFF, 0xFF, game_patch::earlyCiTS,
                                    event::LoadEventAccuracy::Stage_Room_Spawn);

        // early Desert
        eventListener->addLoadEvent(stage::allStages[Stage_Lake_Hylia], 0xFF, 0xFF, 0xFF, 0xFF, game_patch::earlyDesert,
                                    event::LoadEventAccuracy::Stage_Room_Spawn);

        // desert Access
        eventListener->addLoadEvent(stage::allStages[Stage_Gerudo_Desert], 0xFF, 0xFF, 0xFF, 0xFF, game_patch::accessDesert,
                                    event::LoadEventAccuracy::Stage_Room_Spawn);
        ...
        eventListener->addLoadEvent(stage::allStages[Stage_Kakariko_Village], 0xFF, 0xFF, 0xFF, 0xFF,
                                    game_patch::sellWaterBombs, event::LoadEventAccuracy::Stage_Room_Spawn);
    ...
    void Mod::procNewFrame()
    {
        ...
        for (u16 i = 0x0; i < 0xD3; i++)
        {
                ...
                if ((gameInfo.unk_5de4[i] == 0x29 || gameInfo.unk_5de4[i] == 0x3D || gameInfo.unk_5de4[i] == 0x69) && zoomResult
        == 0)
                {
                        zoomResult = i;
                }
                if ((gameInfo.unk_5de4[i] == 0x5E || gameInfo.unk_5de4[i] == 0x8 || gameInfo.unk_5de4[i] == 0x80) && checkResult
        == 0)
                {
                        checkResult = i;
                }
        ...
        else if (enableQuickTransform == 1 && gameInfo.rButtonText == 0 &&
                 (gameInfo.bButtonText == 0x3 || gameInfo.bButtonText == 0x26) &&
                 ((gameInfo.scratchPad.eventBits[0xD] & 0x4) != 0) &&
                 controller::checkForButtonInputSingleFrame(controller::PadInputs::Button_R))
        {
        ...
        if (tp::d_a_alink::checkStageName(stage::allStages[Stage_Lake_Hylia]))
        {
            if (tools::checkItemFlag(ItemFlags::Aurus_Memo))
            {
                gameInfo.scratchPad.itemWheel.Story = items::Item::Aurus_Memo;
                 ...
                 tp::d_a_alink::checkStageName(stage::allStages[Stage_Kakariko_Graveyard]) ||
        ...
        if (isStageShop())
        {
            ...
            {  // normal shops
                ...
                {  // is in shop and is selecting an item
                    ...
                    if (!tools::checkItemFlag(ItemFlags::Null_DA) && bombBagTrickOn == 0 &&
